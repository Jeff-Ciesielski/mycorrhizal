...
class ProcessedTaskToken(Token):
    ...
    def __init__(self, task_id: str, result: str, processing_time: float):
        super().__init__({"task_id": task_id, "result": result, "processing_time": processing_time})
        self.task_id = task_id
        self.result = result
...
class TaskGeneratorInterface(Interface):
    ...
    class TaskSourcePlace(IOInputPlace):
        ...
        def on_input(self) -> Optional[TaskToken]:
    ...
    class MoveTasksTransition(Transition):
        ...
        def output_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.TO_QUEUE: Arc(TaskGeneratorInterface.TaskQueuePlace)
            }
...
class TaskProcessorInterface(Interface):
    ...
    class ProcessTasksTransition(Transition):
        ...
        def output_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.TO_PROCESSING: Arc(TaskProcessorInterface.ProcessingPlace),
...
class NotificationInterface(Interface):
    ...
    class SendNotificationTransition(Transition):
        ...
        def input_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.FROM_INPUT: Arc(NotificationInterface.NotificationInputPlace)
            }
...
class ErrorHandlingInterface(Interface):
    ...
    class HandleErrorsTransition(Transition):
        ...
        def output_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.TO_LOG: Arc(ErrorHandlingInterface.ErrorLogPlace)
            }
...
class TaskProcessingSystem(PetriNet):
        ...
        def on_token_added(self, token: Token) -> Optional[Token]:
            if isinstance(token, ProcessedTaskToken):
                ...
                if net:
                    ...
                    print(f"📊 Progress: {net.tasks_completed}/{net.total_tasks_to_process} tasks completed")
    ...
    class ConnectGeneratorToProcessorTransition(Transition):
        ...
        def input_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.FROM_GENERATOR: Arc(TaskProcessingSystem.TaskGen.TaskQueuePlace)
            }
        
        def output_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.TO_PROCESSOR: Arc(TaskProcessingSystem.TaskProc.TaskInputPlace)
            }
    ...
    class ConnectProcessorToNotificationTransition(Transition):
        ...
        def input_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.FROM_COMPLETED: Arc(TaskProcessingSystem.TaskProc.CompletedPlace)
            }
        
        def output_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.TO_NOTIFICATION: Arc(TaskProcessingSystem.Notify.NotificationInputPlace),
    ...
    class ConnectProcessorToErrorHandlingTransition(Transition):
        ...
        def input_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.FROM_FAILED: Arc(TaskProcessingSystem.TaskProc.FailedPlace)
            }
        
        def output_arcs(self) -> Dict[PlaceName, Arc]:
            return {
                self.ArcNames.TO_ERROR_HANDLER: Arc(TaskProcessingSystem.ErrorHandle.ErrorInputPlace),
                self.ArcNames.TO_TRACKER: Arc(TaskProcessingSystem.CompletionTracker)
            }
...
def demonstrate_comprehensive_system():
    ...
    print(f"- TaskGen: {len([p for p in system._places_by_qualified_name if 'TaskGen' in p])} places")
    print(f"- TaskProc: {len([p for p in system._places_by_qualified_name if 'TaskProc' in p])} places")
    print(f"- Notify: {len([p for p in system._places_by_qualified_name if 'Notify' in p])} places")
    print(f"- ErrorHandle: {len([p for p in system._places_by_qualified_name if 'ErrorHandle' in p])} places")
    ...
    try:
        ...
        system.finished_event.wait(timeout=60.0)
