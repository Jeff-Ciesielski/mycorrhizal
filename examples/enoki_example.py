...
class NetworkTransitions(TransitionEnum):
    SUCCESS = "success"
    FAILURE = "failure"
    TIMEOUT = "timeout"
...
class NetworkRequestState(State):
    ...
    def transitions(cls):
        return {
            ...
            NetworkTransitions.TIMEOUT: GlobalTransitions.RETRY,
    ...
    def on_timeout(cls, ctx: SharedContext) -> TransitionEnum:
        ctx.log("Network request timed out")
        return GlobalTransitions.RETRY
    
    @classmethod
    def on_fail(cls, ctx: SharedContext) -> TransitionEnum:
        ctx.log("Network request failed after all retries")
        return NetworkTransitions.FAILURE
...
class TakePicture(ImagingState):
    ...
    @classmethod
    def on_timeout(cls, ctx: SharedContext) -> TransitionEnum:
        ctx.log("Picture capture timed out")
        return GlobalTransitions.RETRY
    
    @classmethod
    def on_fail(cls, ctx: SharedContext) -> TransitionEnum:
        ctx.log("Picture capture failed after all retries")
        return ImagingTransitions.FAILED
...
class MainMenu(State):
    ...
    def transitions(cls):
        return {
            MenuTransitions.ENTER_SUBMENU: Push("enoki_example.SubMenu", "enoki_example.SubMenuItem"),
...
class SubMenu(State):
    ...
    def transitions(cls):
        return {
            ...
            MenuTransitions.ENTER_SUBMENU: Push("enoki_example.SubMenuItem"),
            GlobalTransitions.UNHANDLED: GlobalTransitions.UNHANDLED
        }
...
class WorkflowTransitions(TransitionEnum):
...
class MainWorkflow(State):
    ...
    def transitions(cls):
        return {
            ...
            WorkflowTransitions.INTERRUPT: Push("enoki_example.InterruptHandler", "enoki_example.MainWorkflow"),
            WorkflowTransitions.START_TASK: Push("enoki_example.TaskExecution", "enoki_example.TaskCleanup", "enoki_example.MainWorkflow"),
...
class TaskExecution(State):
    ...
    def transitions(cls):
        return {
            ...
            WorkflowTransitions.INTERRUPT: Push("enoki_example.InterruptHandler", "enoki_example.TaskExecution"),
            GlobalTransitions.TIMEOUT: GlobalTransitions.UNHANDLED,
            GlobalTransitions.UNHANDLED: GlobalTransitions.UNHANDLED
        }
...
def example_push_pop_detailed():
    ...
    for pop in interrupt_info.get('pop_targets', []):
        if pop.get('is_multi'):
            print(f"  - {pop['transition']}: Can pop to {pop['targets']} (multiple targets)")
        elif pop.get('error'):
...
class DeepTransitions(TransitionEnum):
    DIVE = "dive"
    SURFACE = "surface"
    ...
    def transitions(cls):
        return {
            DeepTransitions.DIVE: Push(
                "enoki_example.Level2",
                "enoki_example.Shared",
                "enoki_example.Level1"
            ),
            GlobalTransitions.UNHANDLED: GlobalTransitions.UNHANDLED
        }
...
def deep_stack_analysis():
    ...
    print(f"  Possible pop destinations: {shared_info.get('possible_pop_destinations', [])}")
...
def example_validation_errors():
    ...
    class BrokenState(State):
        ...
        def transitions(cls):
            return {
                GlobalTransitions.UNHANDLED: "nonexistent.module.BadState"
            }
